	...
	<nav class="main-sidebar">
		...
		<div class="sidebar-wrapper">
			...
			<ul class="sidebar-menu">
					...
					<li class="treeview menuitem-site <?= $before === null ? 'before' : '' ?>">
						...
						<div class="tree-menu-wrapper">
							...
							<div class="menu-body vue" data-key="sidebar-sites">
								<site-tree v-bind:promise="Aimeos.options" current="<?= $enc->attr($this->pageSiteItem->getId()) ?>" parent="<?= $enc->attr($this->pageSitePath->getParentId()->first('0')) ?>" placeholder="<?= $enc->attr($this->translate('admin', 'Find site')) ?>" url="<?= $enc->attr($this->url($searchTarget, $cntl, $action, array('site' => '_code_') + $params, [], $config)) ?>">
								</site-tree>
							</div>
						</div>
					</li>
				...
				<?php foreach ($navlist as $nav => $navitem) : ?>
									...
									<?php foreach (map($navitem)->remove('')->ksort() as $subresource) : ?>
										<?php if ($this->access($this->config('admin/jqadm/resource/' . $subresource . '/groups', []))) : ?>
											...
											<li class="menuitem-<?= str_replace('/', '-', $subresource) ?> <?= $subresource === $resource ? 'active' : '' ?>">
												<a class="item-group" href="<?= $enc->attr($this->url($searchTarget, $cntl, $action, ['resource' => $subresource] + $params, [], $config)) ?>" title="<?= $enc->attr(sprintf($this->translate('admin', '%1$s (Ctrl+Alt+%2$s)'), $this->translate('admin', $subresource), $key)) ?>" data-ctrlkey="<?= $enc->attr(strtolower($key)) ?>">
													<i class="icon"></i>
					...
					<?php else : ?>
						...
						<li class="menuitem-<?= $enc->attr($navitem) ?> <?= $navitem === $before ? 'before' : '' ?> <?= !strncmp($resource, $navitem, strlen($navitem)) ? 'active' : '' ?> <?= $navitem === $after ? 'after' : '' ?>">
							<a class="item-group" href="<?= $enc->attr($this->url($searchTarget, $cntl, $action, array('resource' => $navitem) + $params, [], $config)) ?>" title="<?= $enc->attr(sprintf($this->translate('admin', '%1$s (Ctrl+Alt+%2$s)'), $this->translate('admin', $navitem), $key)) ?>" data-ctrlkey="<?= $enc->attr(strtolower($key)) ?>">
								<i class="icon"></i>
				...
				<?php endif ?>
				<!-- <li class="none">
					<a href="http://127.0.0.1:8000/"> Categories Attributes</a>
				</li> -->
				<li class="none"></li>
			</ul>
